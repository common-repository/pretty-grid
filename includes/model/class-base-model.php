<?php
/**
 * Pretty_Grid_Form_Base_Model Class
 *
 * @since  1.0.0
 * @package Auto Mail
 */
...
if ( ! class_exists( 'Pretty_Grid_Form_Base_Model' ) ) :

	abstract class Pretty_Grid_Form_Base_Model {

		const META_KEY = 'pretty_grid_form';
		...
		public function save( $clone = false, $title = '' ) {
			...
			if( post_exists($title) && is_null($this->id) ){
				return new WP_Error( 'email_exists', sprintf( esc_html__( 'The email "%s" already exists.', Pretty_Grid::DOMAIN ), $title ) );
			}
		...
		/**
		 * @since 1.0.0
		 * @return Pretty_Grid_Form_Field_Model[]
		 */
		 ...
		 * @param $slug
		 *
		 * @return Pretty_Grid_Form_Field|null
		 */
		...
		public function get_all_paged( $current_page = 1, $per_page = null, $status = '' ) {
			if ( is_null( $per_page ) ) {
				$per_page = pretty_grid_form_view_per_page();
			}
			$args = array(
				'post_type'      => $this->post_type,
				'post_status'    => 'any',
     ...
     */
						...
						if ( 'post' === $map['type'] ) {
							...
							if ( ! empty( $meta['fields'] ) && 'fields' === $map['field'] ) {
								foreach ( $meta['fields'] as $field_data ) {
									$field          = new Pretty_Grid_Form_Field_Model( $form_settings );
		...
		public function get_fields_grouped() {
			...
			foreach ( $this->fields as $field ) {
				/** @var Pretty_Grid_Form_Field_Model $field */
